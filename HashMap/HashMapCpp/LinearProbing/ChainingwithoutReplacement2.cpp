/* Hashing */
/* Program DEtails - Hashing, handle collision using linear porbing with chaining */
#include <iostream>
using namespace std;
#define SIZE 10 /* size of the hash function */
#define FALSE 0
#define TRUE 1
...
{
    int i=0,j,start;
    start=h(x); /*hashed lcoation*/
    /*location the beginning of the chain*/
    ...
    while(chain[start]!=-1)
        start=chain[start];
    /* locate an empty place for the current place */
    j=start;
    while(flag[j] && i<SIZE)
    {
        j=(j+1)%SIZE;
